...
func TestConsumePipelineSavePerStep(t *testing.T) {
	t.Parallel()
	...
	testCases := []struct {
	...
	}{
		{
			...
			CancelContext:   false,
			InitSaveTimeout: 0,
			GivenPipeline: pipeline.Trigger(
				"id",
		...
		{
			Name:            "pipeline_consumed_wo_context_cancellation",
			CancelContext:   false,
			InitSaveTimeout: 1 * time.Second,
			GivenPipeline: pipeline.Trigger(
				"id",
	...
	for _, c := range testCases {
		...
		t.Run(c.Name, func(t *testing.T) {
			...
			policy.ConsumePipeline(ctx, c.GivenPipeline)
		})
	}
}
