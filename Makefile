...
gen-api-v1:
	oapi-codegen -generate types -o internal/core/interface/rest/v1/openapi_type.gen.go -package v1 $$API_V1
	oapi-codegen -generate chi-server -o internal/core/interface/rest/v1/openapi_server.gen.go -package v1 $$API_V1
...
test-integration:
	make run-test-db
	make run-test-nats
	MallocNanoZone=0 go test -v -race -coverpkg=./... -coverprofile=integration.out ./internal/infrastructure/... ./internal/config/... || (make stop-test-nats && make stop-test-db && exit 1)
	make stop-test-db
	make stop-test-nats
